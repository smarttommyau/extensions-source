            ...
            if (it.data().contains("subName")) {
                ...
                data.split("},").forEach { ch ->
                    val chapter = SChapter.create().apply {
                        ...
                        if (ch.substringAfter("status").substringBefore(",").replace(Regex("[\"{}:]"), "").replace("\\", "") != "0") {
                            name += " [VIP]"
                            name += " - "
                            name += ch.substringAfter("daysToChangeStatus").substringBefore(",").replace(Regex("[\"{}:]"), "").replace("\\", "")
                        }
...
//        Log.i("manga", response.asJsoup().html())
        val myJsoup = response.asJsoup()
        val head = myJsoup.head()
        val document = myJsoup.selectFirst("section")!!
        return SManga.create().apply {
            title = document.selectFirst("img")!!.attr("alt")
            thumbnail_url = head.selectFirst("meta[property=og:image]")!!.attr("content")
            if (!thumbnail_url.isNullOrEmpty() && thumbnail_url?.contains(baseUrl) != true) {
                thumbnail_url = "$baseUrl/_next/image".toHttpUrl().newBuilder()
                    .addEncodedQueryParameter("url", thumbnail_url)
